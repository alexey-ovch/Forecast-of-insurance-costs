...
Применяется **one-hot encoding** для категориальных переменных `sex`, `smoker`, `region`.
...
Числовые признаки `age` и `bmi` стандартизируются с помощью **z-score**.
...
Таргет-переменная `charges` логарифмируется для уменьшения влияния выбросов и лучшей работы модели.
...
Используется **K-Fold Cross-Validation** с 11 фолдами
...
Модель генерирует предсказания для тестовой выборки, которые затем обратно преобразуются из логарифмической шкалы.

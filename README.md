...
1. Поместите файлы `train.csv` и `test.csv` в директорию, указанную в переменной `PATH_DATA`:
PATH_DATA = './data/' # или другой путь

2. Запустите Jupyter Notebook или Python-скрипт:
jupyter notebook Insurance_costs.ipynb

или
python insurance_costs.py

3. Результат будет сохранен в файл `submission.csv` в виде:
id,charges
770,9194.208538
771,5723.490300
...
...
- **train.csv** — обучающая выборка (не включена в репозиторий)
- **test.csv** — тестовая выборка (не включена в репозиторий)
- **submission.csv** — файл с финальными предсказаниями (генерируется автоматически)
